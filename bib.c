    ...
    if (instrucao != NULL) {
      ...
      if (len == 16 && i < MEM) {
        ...
        if (valido != 0) {
          ...
          cpu->memoria[i].tipo = tipo_outros;
          i++;
        } else {
...
void volta_ciclo(CPU *cpu) {
  ...
  printf("Voltou 1 ciclo. Estado atual: %d (%s), PC=%d\n", cpu->estado_atual,
         NOME_ESTADO[cpu->estado_atual], cpu->pc);
}
...
void disassembla(memoria *mem, char *buf) {
  ...
  case 0:
    switch (mem->funct) {
    case 0:
      sprintf(buf, "add $r%d, $r%d, $r%d", mem->rd, mem->rs, mem->rt);
      break;
    case 1:
      sprintf(buf, "sub $r%d, $r%d, $r%d", mem->rd, mem->rs, mem->rt);
      break;
    case 2:
      sprintf(buf, "and $r%d, $r%d, $r%d", mem->rd, mem->rs, mem->rt);
      break;
    case 3:
      sprintf(buf, "or  $r%d, $r%d, $r%d", mem->rd, mem->rs, mem->rt);
      break;
    default:
      sprintf(buf, "Instrução desconhecida");
      break;
    }
    break;
  case 2:
    sprintf(buf, "j %d", mem->addr);
    break;
  case 4:
    sprintf(buf, "addi $r%d, $r%d, %d", mem->rt, mem->rs, mem->imm);
    break;
  case 8:
    sprintf(buf, "beq $r%d, $r%d, %d", mem->rs, mem->rt, mem->imm);
    break;
  case 11:
    sprintf(buf, "lw $r%d, %d($r%d)", mem->rt, mem->imm, mem->rs);
    break;
  case 15:
    sprintf(buf, "sw $r%d, %d($r%d)", mem->rt, mem->imm, mem->rs);
    break;
  default:
    sprintf(buf, "Instrução desconhecida");
    break;
...
void print_est(CPU *cpu) {
  ...
  printf("Tipo R: %d  (add=%d, sub=%d, and=%d, or=%d)\n", cpu->est.total_r,
         cpu->est.add, cpu->est.sub, cpu->est.and_op, cpu->est.or_op);
  printf("Tipo I: %d  (addi=%d, lw=%d, sw=%d, beq=%d)\n", cpu->est.total_i,
         cpu->est.addi, cpu->est.lw, cpu->est.sw, cpu->est.beq);
